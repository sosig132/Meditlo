                ...
                <div class="bg-gray-700 p-4 rounded-lg shadow-md w-full flex flex-row">
                    <div class="avatar h-10 w-10 mr-4">
                        <div class="w-10 rounded-full">
                            <img src="{{ $user->profile->photo ? Storage::url($user->profile->photo) : 'https://adaptcommunitynetwork.org/wp-content/uploads/2023/09/person-placeholder-450x330.jpg' }}" alt="Profile Photo">
                        </div>
                    </div>
                        ...
                        <div class="flex flex-row gap-3">
                            @foreach ($user->answers as $answer)
                                @if ($answer->possibleAnswer->question_number == 2)
                                    <p class="bg-emerald-600  text-gray-100 rounded px-2">{{ $answer->possibleAnswer->answer }}</p>
                                @endif
                            @endforeach
                        </div>
                    </div>
                    <button class="btn btn-primary ml-auto" wire:click="showUserModal({{ $user->id }})">Trimite cerere</button>
                </div>
            @endforeach
        </div>
    </div>
    <div>
        ...
        <dialog id="user_modal" class="modal modal-bottom sm:modal-middle" wire:ignore.self>
            <div class="modal-box flex flex-col">
                <button wire:click="closeUserModal" class="absolute top-0 right-0 p-2 text-gray-500 hover:text-gray-800 btn btn-ghost">
                    <span class="text-2xl">&times;</span> <!-- The X character -->
                </button>
                <div class="avatar h-20 w-20 mx-auto">
                    <div class="w-20 h-20 rounded-full">
                        <img src="{{ $modalUser?->profile->photo ? Storage::url($modalUser->profile->photo) : 'https://adaptcommunitynetwork.org/wp-content/uploads/2023/09/person-placeholder-450x330.jpg' }}" alt="Profile Photo">
                    </div>
                </div>
                ...
                <div class="mt-2 flex flex-row gap-3 justify-center text-nowrap">
                    @foreach ($user->answers as $answer)
                        @if ($answer->possibleAnswer->question_number == 4)
                            <p class="bg-amber-600  text-gray-100 rounded px-2">{{ $answer->possibleAnswer->answer }}</p>
                        @endif
                    @endforeach
                </div>
                <div class="mt-2 flex flex-row gap-3 justify-center">
