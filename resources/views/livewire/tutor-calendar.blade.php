<div>
    ...
    <div class="modal-box bg-gray-800 text-gray-100" x-data="{ showMaxStudents: false }">
      ...
      <form wire:submit.prevent="createSession">
        <div class="form-control w-full mb-4">
          ...
          <input type="text" wire:model.defer="title" class="input input-bordered w-full bg-gray-700 text-gray-100"
        ...
        <div class="form-control w-full mb-4">
          ...
          <textarea wire:model.defer="description" class="textarea textarea-bordered w-full bg-gray-700 text-gray-100"
        ...
        <div class="form-control w-full mb-4">
          ...
          <select wire:model.defer="type" x-on:change="showMaxStudents = $event.target.value === 'group'"
        ...
        <div class="form-control w-full mb-4" x-show="showMaxStudents" x-transition>
          <label class="label">
            <span class="label-text text-gray-300">Maximum Students</span>
          ...
          <div class="flex items-center gap-2">
            ...
            <input type="number" wire:model.defer="max_students" min="2" max="20"
              class="input input-bordered w-24 bg-gray-700 text-gray-100 text-center" />
          </div>
        ...
        <div class="form-control w-full mb-4">
          ...
          <input type="number" wire:model.defer="price" min="0" step="0.01"
            class="input input-bordered w-full bg-gray-700 text-gray-100" placeholder="Price per student">
  ...
  <dialog id="session_details_modal" class="modal" wire:ignore.self>
    ...
    @endif
      ...
      <div class="mb-4">
      <p class="text-gray-300">
      <strong>Available Spots:</strong> {{ $selectedSession->getAvailableSpots() }} /
      {{ $selectedSession->max_students }}
      </p>
      </div>
    @endif
    ...
    @endif
      ...
      <div class="mb-4">
      <p class="text-gray-300">
      <strong>Recurring:</strong> {{ ucfirst($selectedSession->recurrence_pattern) }} until
      {{ $selectedSession->recurrence_end_date->format('F j, Y') }}
      </p>
      </div>
    @endif
      ...
      <li>
        ...
        @if($selectedSession->price && $selectedSession->price > 0)
          <span class="ml-2">
            @if($participant->hasPaid())
              <span class="badge badge-success">Paid</span>
            @elseif($participant->isPaymentPending())
    ...
    @endif
      ...
      <div class="modal-action">
      <button onclick="session_details_modal.close()" wire:click="openEditModal({{ $selectedSession->id }})"
      class="btn btn-primary">Edit Session</button>
    ...
    @endif
      ...
      @if($selectedSession->isStudentRegistered(auth()->id()))
        @php
          $participant = $selectedSession->participants()->where('student_id', auth()->id())->first();
        @endphp
        
        @if($selectedSession->price && $selectedSession->price > 0 && !$participant->hasPaid())
          <div class="w-full mb-4 p-4 bg-yellow-900/20 border border-yellow-600 rounded-lg">
            <p class="text-yellow-300 text-sm">
              <i class="fas fa-exclamation-triangle mr-2"></i>
              Payment required: ${{ number_format($selectedSession->price, 2) }}
            </p>
            <a href="{{ route('payment.form', $selectedSession->id) }}" 
        ...
        <button wire:click="cancelRegistration({{ $selectedSession->id }})" onclick="session_details_modal.close()"
        class="btn btn-error" wire:loading.attr="disabled">
        Cancel Registration
        </button>
    @else
      <button wire:click="registerForSession({{ $selectedSession->id }})" class="btn btn-primary"
      wire:loading.attr="disabled" @if(!$selectedSession->canAcceptMoreStudents()) disabled @endif>
      <span wire:loading.remove wire:target="registerForSession({{ $selectedSession->id }})">
        @if($selectedSession->price && $selectedSession->price > 0)
          Register & Pay ${{ number_format($selectedSession->price, 2) }}
        @else
